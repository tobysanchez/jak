...
onStart: async function ({ event, message, api, usersData, args}) {
  ...
  if(args[0] == "close") {
if(!global.game.hasOwnProperty(event.threadID) || global.game[event.threadID].on == false ){ message.reply("لا توجد لعبة قيد التشغيل في هذه المجموعة")
  } else {
if(event.senderID == global.game[event.threadID].player1.id || event.senderID == global.game[event.threadID].player2.id ){
  if(event.senderID == global.game[event.threadID].player1.id){
    message.reply({body:`يا له من طفل يبكي. ${global.game[event.threadID].player1.name} ترك اللعبة.\nالفائز هو ${global.game[event.threadID].player2.name}.`, mentions: [{
    ...
    message.reply({body:`يا له من طفل يبكي. ${global.game[event.threadID].player2.name} ترك اللعبة.\nالفائز هو ${global.game[event.threadID].player1.name}.`, mentions: [{
                        tag: global.game[event.threadID].player1.name,
                        id: global.game[event.threadID].player1.id,
 ...
 message.reply("ليس لديك أي لعبة قيد التشغيل في هذه المجموعة")
      ...
      if(mention.length == 0) return message.reply("يرجى ذكر شخص ما ليلعب معك أو قول لعبة قريبة لإغلاق أي لعبة موجودة");
...
message.send(global.game[event.threadID].board)
  ...
  setTimeout(function (){message.send("لا توجد مربعات متبقية. لا أحد يستطيع الفوز بهذه المباراة")}, 1000)
  global.game[event.threadID].on = false
} else{
...
} else{message.reply("هذا المربع ليس فارغا")}

} else{message.reply("يمكن فقط الرد بالأرقام من 1 الى 9")}
