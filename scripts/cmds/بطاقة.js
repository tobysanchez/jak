module.exports = {
  ...
  onStart: async function ({ api, event, args, usersData }) {
    ...
    if (!text1) return api.sendMessage('💢الرجاء إدخال التنسيق الصحيح [text1 - text2] ', event.threadID, event.messageID);
    const text2 = text.split(" - ").pop()
    if (!text2) return api.sendMessage('💢الرجاء إدخال التنسيق الصحيح [text1 - text2] ', event.threadID, event.messageID);
